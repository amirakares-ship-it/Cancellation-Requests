                    ...
                    <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                      <td className="py-2.5 px-3 text-center">
                        <button
                          type="button"
                          onClick={() => setActiveViewerAttachment(item)}
                          className={`inline-flex items-center gap-1 px-2 py-1 rounded-lg font-bold text-[11px] cursor-pointer transition-colors ${isPdf ? 'bg-rose-50 text-rose-700 hover:bg-rose-100' : 'bg-blue-50 text-blue-700 hover:bg-blue-100'}`}
                        >
                          {isPdf ? <FileText className="w-3.5 h-3.5" /> : <ImageIcon className="w-3.5 h-3.5" />}
                          <span>{isPdf ? 'PDF' : 'صورة'}</span>
                        </button>
                      </td>
