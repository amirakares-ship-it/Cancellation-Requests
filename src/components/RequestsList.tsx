...
export default function RequestsList({ 
  ...
  const [isEditingRejectionReason, setIsEditingRejectionReason] = useState(false);
  const [editedRejectionReason, setEditedRejectionReason] = useState('');
  const [isSavingRejectionReason, setIsSavingRejectionReason] = useState(false);
  const [uploadModalTarget, setUploadModalTarget] = useState<any | null>(null);
  ...
  const handleOpenRejectionModal = (r: any) => {
    ...
    setEditedRejectionReason(getRejectionReason(r));
  ...
  const handleSaveRejectionReason = async () => {
    ...
    try {
      ...
      const updatedReason = editedRejectionReason.trim();
      const res = await fetch(`/api/requests/${rejectionModalTarget.id}`, {
        method: 'PUT',
        headers: {
        ...
        body: JSON.stringify({
          ...
          firstManagerComments: updatedReason,
          adminNote: updatedReason
        })
      });
      const data = await res.json();
      if (!res.ok || data.error) {
        throw new Error(data.error || 'فشل حفظ سبب الرفض');
      }
      setRejectionModalTarget({
        ...rejectionModalTarget,
        rejectionReason: updatedReason,
        firstManagerComments: updatedReason,
        adminNote: updatedReason
      ...
      alert(err.message || 'حدث خطأ أثناء حفظ سبب الرفض');
    } finally {
  ...
  const getCommitteeDecisionBadge = (r: any) => {
    ...
    if (isAccepted) {
      return (
        <span
          className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xxs font-black bg-emerald-100 text-emerald-800 border border-emerald-300 shadow-2xs"
          title="تم الاعتماد"
        >
    ...
    return (
      <span
        className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xxs font-bold bg-amber-50 text-amber-800 border border-amber-200 shadow-2xs"
        title="قيد الانتظار"
      >
  ...
  const [financeDateTarget, setFinanceDateTarget] = useState<any | null>(null);
  ...
  // but what's actually shown to the user for that state is
  ...
  const handleOpenFinanceDateEdit = (r: any) => {
  ...
  const handleClearFinanceSent = async (r: any) => {
    try {
      const token = localStorage.getItem('wd_token') || '';
  ...
  const handleToggleFinanceSent = (r: any) => {
  ...
  return (
    <div className="space-y-4 text-right" dir="rtl">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {onExportExcel && (
              <button
                ...
                className="flex items-center gap-1.5 px-3.5 py-2 bg-slate-800 hover:bg-slate-700 text-amber-400 font-black text-xs rounded-xl transition-all shadow-sm hover:shadow cursor-pointer active:scale-95 border border-slate-700"
              >
            ...
            {user.role === 'admin' && onBulkReview && (
              <>
                ...
                <button
                  ...
                  className="px-4 py-2 bg-slate-700 hover:bg-slate-600 text-amber-400 font-bold text-xs rounded-lg transition-colors cursor-pointer border border-slate-600"
                >
                    ...
                    >
                      ...
                      <td className="py-3.5 px-4 text-center">
                        <div className="flex items-center justify-center gap-1.5">
                          ...
                          <button
                            type="button"
                            onClick={() => setUploadModalTarget(r)}
                            className={`p-1.5 text-xxs font-bold rounded-md flex items-center gap-1 cursor-pointer transition-colors shadow-2xs ${
                              (r.attachments || []).length > 0
                                ? 'bg-purple-50 hover:bg-purple-100 text-purple-800 border border-purple-300'
                                : 'bg-slate-50 hover:bg-slate-100 text-slate-600 border border-slate-200'
                            }`}
                            title={`المستندات والمرفقات الرسمية (${(r.attachments || []).length} مرفق)`}
                          >
                            <Paperclip className={`h-3.5 w-3.5 ${(r.attachments || []).length > 0 ? 'text-purple-600' : 'text-slate-400'}`} />
                            <span>{(r.attachments || []).length > 0 ? `${r.attachments.length} مرفق` : '+ مستند'}</span>
                          </button>
          ...
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 space-y-4 shadow-2xl border border-slate-100 animate-in fade-in zoom-in-95">
            ...
            <div className="bg-rose-50/80 p-4 rounded-xl border border-rose-200 space-y-2">
              <div className="flex items-center justify-between">
                ...
                {(user?.role === 'admin' || user?.role === 'first_manager' || user?.role === 'sector_manager') && !isEditingRejectionReason && (
                  <button
                    type="button"
                    onClick={() => setIsEditingRejectionReason(true)}
                    className="text-xxs font-bold text-rose-700 hover:text-rose-900 bg-white hover:bg-rose-100 px-2.5 py-1 rounded-lg border border-rose-200 flex items-center gap-1 cursor-pointer transition-colors"
                  >
                    <Edit3 className="w-3 h-3" />
                    <span>تعديل سبب الرفض</span>
                  </button>
                )}
              </div>
              ...
              {isEditingRejectionReason ? (
                <div className="space-y-2 pt-1">
                  <textarea
                    ...
                    placeholder="اكتب سبب الرفض الحقيقي هنا (مثال: عدم استيفاء الأوراق، تجاوز المدة القانونية، إلخ)..."
                    className="w-full bg-white border border-rose-300 rounded-xl p-2.5 text-xs text-slate-800 focus:outline-none focus:ring-2 focus:ring-rose-400 font-sans shadow-inner"
                    autoFocus
                  />
                  <div className="flex items-center justify-end gap-2">
                    <button
                      type="button"
                      onClick={() => setIsEditingRejectionReason(false)}
                    ...
                    <button
                      ...
                      className="px-4 py-1.5 bg-rose-600 hover:bg-rose-700 text-white font-black text-xs rounded-lg flex items-center gap-1.5 cursor-pointer shadow-xs disabled:opacity-50"
                    >
                      {isSavingRejectionReason ? (
                        <span>جاري الحفظ...</span>
                      ) : (
                        <>
                          <Check className="w-3.5 h-3.5" />
                          <span>حفظ سبب الرفض</span>
                        </>
                      )}
                    </button>
                  </div>
                ...
                <div className="bg-white p-3.5 rounded-xl border border-rose-100 text-slate-800 text-xs font-bold leading-relaxed whitespace-pre-wrap min-h-[48px] flex items-center">
                  {getRejectionReason(rejectionModalTarget) || (
                    <span className="text-slate-400 font-normal italic">
                      لم يتم تسجيل سبب محدد للرفض بعد.
                    </span>
                  )}
                </div>
              )}
      ...
      {/* Standalone Attachment Viewer Modal */}
